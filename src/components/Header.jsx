...
const HeaderComponent = () => {
  ...
  useEffect(() => {
    const onScroll = () => {
      const above = window.scrollY > 50;
      if (scrolledRef.current !== above) {
        scrolledRef.current = above;
      ...
      <div className="container-custom mx-auto px-4 flex justify-between items-center">
        ...
        <nav className="hidden md:flex items-center space-x-6">
          {navLinks.map((link) => (
            ...
            >
              <GlitchText text={link.label} interactive={false} intensity="low" glitchStyle="classic" />
            </Link>
          ))}
        </nav>
